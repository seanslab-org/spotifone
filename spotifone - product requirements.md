...
1. Bluetooth Microphone
   Use HFP (Hands-Free Profile) and other standard Bluetooth protocols to enable audio input for both Mac and Windows.

2. Bluetooth Keyboard
   (a) Use BLE (Bluetooth Low Energy) to make it a standard Bluetooth keyboard that can send keys to Mac and Windows.
   (b) Configure the round button on the screen (the only button on the screen, not the wheel) to be a push-to-talk button.
